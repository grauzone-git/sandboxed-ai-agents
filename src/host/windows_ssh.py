...
class SshSetup:
    ...
    def read_config(self):
        # Provisioning can be lengthy; recheck redirection at the time of access.
        for path in (self.home / '.ssh', self.root, self.config):
            if path.is_symlink() or (path.exists() and
                    getattr(path.lstat(), 'st_file_attributes', 0) & stat.FILE_ATTRIBUTE_REPARSE_POINT):
                raise ValueError('SSH setup refuses symlinks and reparse points in managed paths.')
        return self.config.read_bytes() if self.config.exists() else None
    ...
    def write(self, path, content, *, expected_config=_UNCHECKED):
        descriptor, temporary = tempfile.mkstemp(prefix='.sandbox-ssh-', dir=path.parent)
        try:
            with os.fdopen(descriptor, 'w', encoding='utf-8', newline='\n') as stream:
                stream.write(content)
            self.secure(Path(temporary))
            if expected_config is not _UNCHECKED and self.read_config() != expected_config:
                raise ValueError('SSH config changed while preparing the update; retry. Existing config was retained.')
    ...
    @contextmanager
    def config_lock(self, *, for_removal=False):
        include = f'Include "{(self.state / (self.name + ".conf")).as_posix()}"'
        current = self.read_config()
        if for_removal and not self.state.exists() and (current is None or
                include not in [line.strip() for line in current.decode('utf-8-sig').splitlines()]):
            yield
            return
        self.root.mkdir(parents=True, exist_ok=True)
        lock = self.root / '.config.lock'
        # A shared, exclusive directory serializes our atomic replacements even
        # across checkouts. Fail rather than guessing that another writer is stale.
        try:
            lock.mkdir()
        except FileExistsError as error:
            raise ValueError(f'SSH config update is locked: {lock}. Retry when the other operation finishes; '
                             'remove the lock only if no SSH setup/removal is running.') from error
        try:
            yield
        finally:
            lock.rmdir()

    def update_include(self, *, install):
        # Call only while holding config_lock (or when there is nothing to remove).
        include = f'Include "{(self.state / (self.name + ".conf")).as_posix()}"'
        original = self.read_config()
        content = original.decode('utf-8-sig') if original is not None else ''
        lines = [line for line in content.splitlines(keepends=True) if line.strip() != include]
        updated = (include + '\n' if install else '') + ''.join(lines)
        if updated != content:
            self.write(self.config, updated, expected_config=original)
...
''')
        with self.config_lock():
            self.update_include(install=True)
        print(f'SSH configured: ssh {self.name}')

    def remove(self, *, config_locked=False):
        if not config_locked:
            with self.config_lock(for_removal=True):
                return self.remove(config_locked=True)
        ...
        self.update_include(install=False)
